...
### Готовые утилиты — используй, не изобретай заново
В `index.css` под `@layer components` есть полный набор переиспользуемых классов. Перед созданием нового UI-элемента сначала проверь, есть ли уже подходящий:
- `.card-base` — карточка с тенью, hover-эффектом и border-radius из `--radius`
- `.btn-primary-landing` / `.btn-secondary-landing` — кнопки с округлыми краями
- `.section-container` — центрированный контейнер max-w-[1280px] с адаптивными отступами
- `.section-padding` — стандартный вертикальный padding секции
- `.pill-badge` / `.pill-accent` — бейджи на primary/accent цвете
- `.kpi-card` — карточка метрики с фоном на primary/4%
- `.gradient-hero` — фоновый градиент для hero-секции

### Dark mode — не активирован, но переменные частично определены
`tailwind.config.ts` использует `darkMode: ["class"]`, а в `index.css` есть блок `.dark { }`. Однако в `.dark` отсутствуют кастомные переменные: `--background-alt`, `--primary-glow`, `--shadow-card`, `--shadow-glow`, `--font-display`, `--font-body`. Это значит: если кто-то включит тёмную тему, кастомные утилиты сломаются визуально. Не активируй dark mode и не добавляй переключатель темы без предварительного аудита и дополнения `.dark`-переменных.

### Навигация и якорные ссылки — как устроена прокрутка
`SiteHeader` использует якорные `href="#section-id"` для навигации. `index.css` задаёт `scroll-padding-top: 80px` для компенсации высоты фиксированного хедера. Каждая секция лендинга имеет `id`: `#segments`, `#features`, `#roadmap`, `#waitlist`. При добавлении новой секции:
1. Добавь `id` атрибут на `<section>`
2. Добавь якорную ссылку в `SiteHeader.tsx`
3. Убедись, что `scroll-padding-top` достаточен (сейчас 80px)
...
6. **Не использовать хардкод-цвета (hex, rgb, oklch) вместо CSS-переменных** — проект построен на дизайн-токенах (`--primary`, `--accent`, `--muted` и др.), которые пробрасываются в Tailwind через `tailwind.config.ts`. Хардкод вида `text-[#6366f1]` или `style={{ color: '#6B5CE7' }}` выпадает из системы: не обновится при смене темы, не найдётся grep'ом по токенам, сломает `--shadow-glow` (тень тоже берёт цвет из переменной). Всегда используй `text-primary`, `bg-primary/10`, `hsl(var(--accent))` и т.п.

7. **Не применять Tailwind-класс `container` вместо кастомного `.section-container`** — `tailwind.config.ts` задаёт `container` с `max-width: 1400px` и `padding: 2rem`, тогда как `.section-container` в `index.css` — `max-width: 1280px` с адаптивными отступами `px-5 md:px-8 lg:px-12`. Использование `container mx-auto` даст более широкий блок с другими отступами — секция визуально выбьется из общей сетки сайта.

8. **Не добавлять CSS-анимации (keyframes, `animate-*`) там, где уже применяется Framer Motion** — весь проект использует единый паттерн `motion.*` + `useInView({ once: true })`. CSS-анимации запустятся сразу при загрузке, не будут ждать скролла и не поддержат координированные задержки (`custom` prop). Два несвязанных слоя анимации создадут визуальный конфликт. Для новых секций всегда используй `motion.div` с `useInView`.

9. **Не создавать кастомные CSS-утилиты вне `index.css`** — в `index.css` под `@layer components` живут все переиспользуемые классы: `.card-base`, `.btn-primary-landing`, `.section-padding`, `.pill-badge`, `.kpi-card` и др. Стили в отдельном `.css`-файле или через `<style>` в компоненте выпадут из каскада, могут конфликтовать с Tailwind-purge и не будут доступны другим компонентам.

10. **Не добавлять роуты после catch-all `"*"` в `App.tsx`** — React Router обрабатывает маршруты сверху вниз. В `App.tsx` есть явный комментарий: `ADD ALL CUSTOM ROUTES ABOVE THE CATCH-ALL "*" ROUTE`. Роут, добавленный после `<Route path="*">`, никогда не будет достигнут — пользователь увидит `NotFound` вместо нужной страницы.
